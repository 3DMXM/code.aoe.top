...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.video.exec(src)) {
      src = src.substring(cap[0].length);
      out += this.renderer.video(escape(this.smartypants(cap[1])));
      continue;
    }
    // url (gfm)
...
// video
Renderer.prototype.video = function (url) {
  // console.log("解析视频");
  url = convert_url(url);
  console.log(url);
  let out = '<iframe height="375" width="500" src="'+url+'" frameborder="no" allowfullscreen="true"></iframe>';
  return out;
}
